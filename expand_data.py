...
############################################################################################
def print_d(input_str, debug=False):
    """ 
    Simple debugging print statements that can be turned off or on with the debug variable. 
    inputs: str, a string to output; debug: true to print statements
    """
    debug = debug
...
def load_images(opt):
    ...
    x_data_paths, y_data_paths = load_data_filenames(opt)
...
def get_filenames(dir, ext):
    ...
    source_regex = os.path.join(dir, wild_card)
...
def load_data_filenames(opt):
    ...
    data_image_files = get_filenames(get_source_dir(opt), opt.ext)
    if opt.label_dir:
        data_label_files = get_filenames(opt.label_dir, opt.ext)
